...
public class UploadProductRequestHandler implements RequestHandler<Product, Product> {
    ...
    public Product handleRequest(Product product, Context context) {
        ...
        log.log("Persisted product: " + updateItemResult.toString());

        return Product.builder()
                .id(updateItemResult.getAttributes().get("id").getS())
    ...
    private UpdateItemResult persistData(Product product) {
        ...
        if (product.getId() != null) {
            ...
            if (item != null)
                throw new RuntimeException("Product with ID: " + item.getString("id") + " already exists");
        }

        return table.updateItem(new UpdateItemSpec().withPrimaryKey("id", UUID.randomUUID().toString())
